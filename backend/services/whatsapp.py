...
# Per-vertical opening messages. {name}/{city} are filled in; sales rep
# personalizes the rest before sending.
...
def build_message(name: str | None, city: str | None, vertical_tag: str) -> str:
    template = TEMPLATES.get(vertical_tag, TEMPLATES["default"])
    return template.format(name=name or "your business", city=city or "your area")
...
def whatsapp_url(
    ...
    if not digits:
        return None
    text = quote(build_message(name, city, vertical_tag))
